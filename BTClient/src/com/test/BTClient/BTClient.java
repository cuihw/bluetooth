...
public class BTClient extends Activity {
    ...
    public void onSendButtonClicked(View v){
    	...
    	if (_socket == null) {
            ...
            Toast.makeText(this, "没有连接到蓝牙设备。", Toast.LENGTH_LONG).show();
            return;
    	}
    	try{
    		OutputStream os = _socket.getOutputStream();   //蓝牙连接输出流
    		String input = edit0.getText().toString();

    		Log.d(TAG, "input = " + input);

    		if (TextUtils.isEmpty(input)) {
    	        Toast.makeText(this, "输入内容为空，不发送！", Toast.LENGTH_LONG).show();
    		    return;
    		}
    		byte[] bos = input.getBytes();
